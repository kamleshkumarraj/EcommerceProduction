...
export async function fetchAllWishlistItem(dispatch, user) {
  ...
  const options = {
    url: `${import.meta.env.VITE_API_URL}/api/v2/user/wishlist/get`,
    method: "GET",
  };
...
export async function updateWishlistQty(
  ...
  const options = {
    url: `${import.meta.env.VITE_API_URL}/api/v2/user/wishlist/update/${wishlistItem._id}?operation=${operation}`,
    method: "PATCH",
  };
...
export async function removeWishlistItem(
  ...
  const options = {
    url: `${import.meta.env.VITE_API_URL}/api/v2/user/wishlist/remove/${wishlistItem._id}`,
    method: "DELETE",
  };
...
export async function addWishlistItem(
  ...
  const options = {
    url: `${import.meta.env.VITE_API_URL}/api/v2/user/wishlist/add/${product._id}`,
    method: "POST",
  };
  setEventLoading(true);
  const response = await dispatch(apiCalling(options));
