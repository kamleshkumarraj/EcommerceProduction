...
function TestCard({title , sender , type}) {
  ...
  return (
    ...
    <div className="text-section w-[100%] ">
      <img className='relative top-[25px] max-w-[20px]' src={comma_l} alt="" />
      
        <p className='text-[15px] text-gray-600 font-[500] leading-[25px] text-center pl-[50px]'>{title} </p>
        
        <p className='relative '><img className='relative left-[400px] top-[-20px] max-w-[20px]' src={comma_r} alt="" /></p>
       
    </div>
    <div className="image-section flex gap-[1rem] items-center mx-auto">
        <img className='max-w-[48px] max-h-[48px] rounded-full' src={user?.avatar?.url} alt="" />
        <div className="text">
