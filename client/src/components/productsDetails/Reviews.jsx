...
const ReviewSection = ({ products, changed }) => {
  ...
  useEffect(() => {
    fetchReviews({ dispatch, productId: products?._id }).then(
      ({ data, success }) => {
        if (success) setReviewsData(data);
        else
          console.log("We get error during fetching reviews from database!");
      }
    );
  }, [changed]);

  const handleReviewRatingSocket = useCallback((data) => {
  ...
  useHandleSocket({[CREATE_REVIEW_RATING] : handleReviewRatingSocket})
  
  return (
    <>
      {reviewsData && (
        <div className="p-[3.2rem] px-[80px]">
          <h2 className="mb-4 text-[2.4rem] font-bold">
            {reviewsData.length} Reviews
          </h2>
          {reviewsData.map((review, index) => (
            <>
              
                {review.comment.map((comment , idx) => (
                  <div key={comment} className="flex items-start mb-8">
                    <img
                      src={
                        review?.avatar ||
                        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRYiQVI64BOhcmcvgwR3TrgsgxkEyWmQCdz9g&s"
                      }
                      alt={"reviewer-photo"}
