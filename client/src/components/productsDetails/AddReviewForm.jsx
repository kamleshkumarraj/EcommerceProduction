...
const AddReviewForm = ({ products, setChanged, changed }) => {
  ...
  const postReviews = async () => {
    const options = {
      method: "PUT",
      formData: reviewData,
      url: `https://ecommerceproduction.onrender.com/api/v2/user/product/give-reviews?id=${products._id}`,
    };
  ...
  return (
    <div className="bg-gray-50 ">
      <div className="w-[100%] px-[100px] p-[2.4rem] mx-auto bg-white rounded shadow-md">
        <h1 className="mb-[1.6rem] text-[2.4rem] font-[500] text-black">
          Add A Review
        </h1>
        ...
        <div className="grid grid-cols-1 gap-[1.6rem] mb-[1.6rem] sm:grid-cols-2"></div>
        <div className="relative mb-[1.6rem]">
          <textarea
            placeholder="Write a Message"
            value={reviewData.comment}
            onChange={(e) =>
              setReviewsData({ ...reviewData, comment: e.target.value })
            }
        ...
        <button
          className="px-[2.4rem] py-[1.2rem] text-white bg-red-500 rounded hover:bg-red-600 focus:ring focus:ring-red-300"
          onClick={postReviews}
        >
          Post A Comment →
        </button>
