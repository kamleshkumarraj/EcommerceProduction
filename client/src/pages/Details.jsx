...
import { getAllProducts, getTopRatedProducts } from "../store/slices/productsHandler.slice";

const Details = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { product } = useLocation().state;
  const moreProducts = useSelector(getTopRatedProducts);
  const [image, setImage] = useState(product?.thumbanail);
  ...
  const [relatedProducts , setQuery] = useFilter(productsList , (data) => data.category)
  const [quantity, setQuantity] = useState(1);
  useEffect(() => {
    setQuery(product.category)
  },[productsList , product , setQuery])
        ...
        <div className="w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto pb-16">
          <div className="grid grid-cols-2 gap-8 md-lg:grid-cols-1">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div id="dub-details" className="grid grid-cols-2">
                  <p>
                ...
                <div id="return-policy">
                    <p className="text-[20px]" id="return">Return Policy : <span className="text-[18px] px-[4rem]">{product.returnPolicy}</span></p>
                    <p className="text-[18px]">Last Order within 10days : {product.minimumOrderQuantity} qty</p>
                    <p className="warranty text-[18px] font-[500]">Warranty : {product.warrantyInformation}</p>
                </div>
                  ...
                  {moreProducts.slice(0,4).map((p, i) => {
                    return (
                      <Link key={i} className="block">
                        ...
                        <h2 className="py-1 text-slate-600">{p.name}</h2>
        ...
        <div className="w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto">
          ...
          <div>
            <Swiper
              ...
              className="mySwiper"
            >
              {relatedProducts && relatedProducts.length > 0 && relatedProducts.slice(0,12).map((p, i) => {
                return (
                  <SwiperSlide key={i}>
                    <Link state={{product : p}} to={`/product-details/${p._id}`} className="block">
                      <div className="relative h-[270px]">
