...
const Cart = () => {
  ...
  const removeCartItem = async (_id) => {
    const options = {
      url: `${import.meta.env.VITE_API_URL}/api/v2/user/cart/remove/${_id}`,
      method: "DELETE",
    };
  ...
  const increamentCartQty = async (_id) => {
    const options = {
      url: `${import.meta.env.VITE_API_URL}/api/v2/user/cart/increase/${_id}`,
      method: "PATCH",
    };
    const response = await dispatch(apiCalling(options));
    if (response?.success) {
  ...
  const decreamentCartQty = async (_id, qty) => {
    const options = {
      url: `${import.meta.env.VITE_API_URL}/api/v2/user/cart/decrease/${_id}`,
      method: "PATCH",
    };
    const response = await dispatch(apiCalling(options));
    if (response?.success) {
